...
![](/image/CreateNewRepositoryAndInit.png)
...
![](/image/GitModifyTestFile.png)
然后我们用 *`git status`* 查看下当前仓库状态。
![](/image/GtiStatusModify.png)
...
![](/image/GitCheckoutDot.png)
上一个执行status命令提示的修改都没有了，在看看工作目录下的文件现在变成了：
![](/image/GitCheckoutTestFile.png)
恢复成了我们修改过之前的内容了。
...
首先，先看下现在文件状态：
![](/image/GitResetFile_1.png)
注意“git测试.txt”的修改日期为：2022-7-19 9:47
我们看到本地仓库现在有两个版本：
![](/image/GitResetFile_2.png)
...
![](/image/GitResetFile_5.png)
看，我们又回到了原来 *`reset`* 之前的状态了！
![](/image/GitResetFile_6.png)
...
我们先在工作目录下创建一个文件：
![](/image/GitDeleteFile_1.png)
然后执行上面两个命令：
![](/image/GitDeleteFile_2.png)
...
我们再来在工作目录下创建一个文件：
![](/image/GitDeleteFile_3.png)
新添加文件处于未被追踪状态，然后我们先提交到暂存区。顺手把“git测试.txt"修改一下，等下可以做下对比。
![](/image/GitDeleteFile_4.png)
我们先执行 *`git ls-files`* 查看暂存区中有哪些文件：
![](/image/GitDeleteFile_5.png)
然后尝试删除它们：
![](/image/GitDeleteFile_6.png)
可以看到，都失败了，提示的错误并不一样：
...
![](/image/Github_RemoteTest_1.png)
然后，我们要拷贝这个SSH以备用：
![](/image/Github_RemoteTest_2.png)
...
![](/image/Github_MainBranch.png)
![](/image/Github_MasterBranch.png)
...
![](/image/GitBranchDel_1.png)
然后切换到delBranch分支：
![](/image/GitBranchDel_2.png)
把delBranch分支推送到远端仓库：
![](/image/GitBranchDel_3.png)
就是这么简单！

### **3. 合并分支**

现在先回到master，我们新建一个gitMaster_MergeTest.txt文件，输入点内容，然后查看下当前状态：
![](/image/GitBranchMerge_1.png)
...
![](/image/GitBranchMerge_2.png)
现在master的版本号已经改变喽。然后，我们回到delBranch分支：
![](/image/GitBranchMerge_3.png)
...
![](/image/GitRemoteBranchDel_1.png)
看到了之前我们介绍创建远程分支时候特别创建的delBranch了。然后我们对它进行删除：
![](/image/GitRemoteBranchDel_2.png)
