...
![](./image/Windows%20Explorer%20Integration.png)
一路Next，在选择默认编辑器时候按照个人需求选择：
![](./image/Default%20Editor%20for%20Git.png)
一路Next，等待安装完成。
...
![](./image/Create%20New%20Repository%20and%20Init.png)
...
![](./image/Git%20Commit_2.png)
设置完成后，现在可以提交了，执行 *`git commit -m "My gitTest V1.0"`*
![](./image/Git%20Commit_3.png)
...
![](./image/Git%20checkout.png)
上一个执行status命令提示的修改都没有了，在看看工作目录下的文件现在变成了：
![](./image/Git%20CheckoutFile.png)
恢复成了我们修改过之前的内容了。
...
首先，先看下现在文件状态：
![](./image/Git%20ResetFile_1.png)
注意“git测试.txt”的修改日期为：2022-7-19 9:47
我们看到本地仓库现在有两个版本：
![](./image/Git%20ResetFile_2.png)
...
![](./image/Git%20ResetFile_3.png)
现在，HEAD已经指向我们指定的索引了，并且我们可以发现，之前的commit都被清除了！此时的文件状态：
![](./image/Git%20ResetFile_4.png)
...
![](./image/Git%20ResetFile_5.png)
看，我们又回到了原来 *`reset`* 之前的状态了！
![](./image/Git%20ResetFile_6.png)
...
我们先在工作目录下创建一个文件：
![](./image/Git%20deleteFile_1.png)
然后执行上面两个命令：
![](./image/Git%20deleteFile_2.png)
...
我们再来在工作目录下创建一个文件：
![](./image/Git%20deleteFile_3.png)
新添加文件处于未被追踪状态，然后我们先提交到暂存区。顺手把“git测试.txt"修改一下，等下可以做下对比。
![](./image/Git%20deleteFile_4.png)
我们先执行 *`git ls-files`* 查看暂存区中有哪些文件：
![](./image/Git%20deleteFile_5.png)
然后尝试删除它们：
![](./image/Git%20deleteFile_6.png)
可以看到，都失败了，提示的错误并不一样：
...
![](./image/Github%20Settings.png)
![](./image/Github%20SSH%20KEY.png)
准备工作完成，我们先来验证下是否成功：
输入命令： ***`ssh -T git@github.com`*** :
![](./image/Git%20SSH_1.png)
...
![](./image/Github%20RemoteTest_1.png)
然后，我们要拷贝这个SSH以备用：
![](./image/Github%20RemoteTest_2.png)
...
![](./image/Github%20Main%20Branch.png)
![](./image/Github%20Master%20Branch.png)
...
![](./image/Git%20Branch_Del_1.png)
然后切换到delBranch分支：
![](./image/Git%20Branch_Del_2.png)
把delBranch分支推送到远端仓库：
![](./image/Git%20Branch_Del_3.png)
就是这么简单！

### **3. 合并分支**

现在先回到master，我们新建一个gitMaster_MergeTest.txt文件，输入点内容，然后查看下当前状态：
![](./image/Git%20Branch_Merge_1.png)
...
![](./image/Git%20Branch_Merge_2.png)
现在master的版本号已经改变喽。然后，我们回到delBranch分支：
![](./image/Git%20Branch_Merge_3.png)
...
![](./image/Git%20Branch_Merge_4.png)
同样的，add新添加的文件然后提交：
![](./image/Git%20Branch_Merge_5.png)
准备工作都做完了，真正开始合并的时候了。我们回到Master分支，然后合并delBranch分支。
执行 ***`git merge [branch-name]`*** 进行合并分支操作：
![](./image/Git%20Branch_Merge_6.png)
内容有点多，我们一个一个分析：
...
![](./image/Git%20Branch_Merge_7.png)
...
![](./image/Git%20Remote_Branch_Del_1.png)
看到了之前我们介绍创建远程分支时候特别创建的delBranch了。然后我们对它进行删除：
![](./image/Git%20Remote_Branch_Del_2.png)
